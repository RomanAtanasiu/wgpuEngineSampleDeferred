...
int SampleRenderer::post_initialize()
{

    {
        ...
        red_in_screen.id = Renderer::post_process_add_render_pass(red_in_screen.shader, AFTER_LIGHT_PASS,{ time_bindgroup });
        red_in_screen.name = "low hp";
    }

    {
        black_and_white.shader = RendererStorage::get_shader("data/test_shaders/black_and_white.wgsl");
        black_and_white.id = post_process_add_render_pass(black_and_white.shader, AFTER_LIGHT_PASS);
...

    {
        ...
        contrast.id = post_process_add_compute_pass(contrast.shader,BEFORE_GAMMA_PASS, "contrast_compute", workgroups);
    ...
    }
    {
        ...
        int workgroups[3] = { webgpu_context->gbuffer_format.width / 8, webgpu_context->gbuffer_format.height / 8 , 1 };
        blur.id = post_process_add_compute_pass(blur.shader, AFTER_LIGHT_PASS,"box_blur", workgroups);
...
void SampleRenderer::render()
{
    ...
    std::vector<tPostProcess> ordered_post_process = Renderer::post_process_get_ids_in_render_order(AFTER_LIGHT_PASS);
    ...
    ImGui::SeparatorText("BEFORE_GAMMA_PASS");
    ordered_post_process = Renderer::post_process_get_ids_in_render_order(BEFORE_GAMMA_PASS);
    ...
    if(ImGui::Button("swap should fail, changing contrast and low hp"))
        Renderer::post_process_swap_passes(red_in_screen.id, contrast.id);
